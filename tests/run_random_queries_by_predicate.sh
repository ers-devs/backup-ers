...
while true
do
	orig_subject=$(shuf -n 1 ${INPUT_FILE} | cut -d " "  -f 2)
	predicate=$(echo $orig_subject | sed -e 's/:/%3A/g' | sed -e 's/#/%23/g')
	#echo $subject
        len=${#predicate} 
	len=$((len - 2))
	rm tmp 2> /dev/null
	# now query the DB 
	{ time curl -X GET 'http://localhost:5984/'$COUCHDB_NAME'/_design/entity/_view/by_predicate?key="'${predicate:1:$len}'"'; } &> tmp
	#cat tmp
