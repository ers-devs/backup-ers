...
while true
do
	orig_object=$(shuf -n 1 ${INPUT_FILE} | cut -d " "  -f 3)
	if [ ${orig_object:0:1} != "<" ]; then
		continue
	fi
	object=$(echo $orig_object | sed -e 's/:/%3A/g' | sed -e 's/#/%23/g')
        len=${#object} 
	len=$((len - 2))
	rm tmp 2> /dev/null
	# now query the DB 
	{ time curl -X GET 'http://localhost:5984/'$COUCHDB_NAME'/_design/entity/_view/by_object?key="'${object:1:$len}'"'; } &> tmp
	#cat tmp
